...
def job_opening_login(request):
    ...
    try:
        ...
        if job_openings_login_details:
            ...
            cur.execute("""
                    select ci.id,job_opening_id,coalesce(first_name||' '||last_name) as name,primary_email,coalesce(to_char(date_of_birth,'dd-mm-yyyy'),'') as date_of_birth,mobile_no,address,
                    coalesce((select job_title from ta_job_openings_info where id=job_opening_id),'') as job_title,
                    coalesce((select job_short_description from ta_job_openings_info where id=job_opening_id),'') as job_short_desc,
                    coalesce((select job_description from ta_job_openings_info where id=job_opening_id),'') as job_desc
                    ,coalesce(to_char(ci.modified_date,'dd/mm/yyyy'),'') as applied_date,ri.refitems_name as job_status,ri1.refitems_name as screening_status,ri2.refitems_name as interview_status,
		    ri3.refitems_name as offer_status,ri4.refitems_name as hired_status 
                    from ta_candidate_info ci left join reference_items ri on ci.candidate_status_id = ri.id  
		    left join reference_items ri1 on ci.screening_id = ri1.id
		    left join reference_items ri2 on ci.interview_id =ri2.id 
		    left join reference_items ri3 on ci.offer_release_id = ri3.id
		    left join reference_items ri4 on ci.hired_id = ri4.id where ci.id = {0} and job_opening_id = {1}
            ...
            cur.execute("""select coalesce(to_char(tci.modified_date,'dd/Mon/yyyy'),'') as date,coalesce(ai.name,'') as attachment from ta_candidate_info tci 
                inner join ta_interview_info tii on tci.id = tii.candidate_name_id
                left join attachment_info ai on ai.id = tii.attachment_id where tci.id='{0}'""".format(int(job_openings_login_details[0]['id'])))
            resume_detail=q.dictfetchall(cur)
            print"RRRRRRRRR",resume_detail[0]['attachment']
            json_data['resume_details']=resume_detail[0]['attachment']
            cur.execute("""
                    select ri.refitems_name as interview_status from ta_interview_info ii left join reference_items ri on ii.interview_type_id = ri.id
                    where candidate_name_id = {0} and job_title_id = {1}
            """.format(int(job_openings_login_details[0]['id']),int(job_openings_login_details[0]['job_opening_id'])))
            interview_current_status = q.dictfetchall(cur)
            print"CDDDDDDDDDD",interview_current_status
            json_data['interview_current_status'] = interview_current_status
            cur.execute("""
            ...
            cur.execute("""
                    select id,coalesce(to_char(created_date,'dd-mm-yyyy'),'') as created_date,is_active,doc_download_name,ctc,doc_mail_status,candidate_id,cost_to_employee_id,created_by_id,job_title_id,modified_by_id,offer_release_id from ta_offer_info where candidate_id = {0} and job_title_id = {1}
            """.format(int(job_openings_login_details[0]['id']),int(job_openings_login_details[0]['job_opening_id'])))
            interview_refer_offer_details = q.dictfetchall(cur)
            json_data['interview_refer_offer_details'] = interview_refer_offer_details
            cur.execute("""
                    select id,coalesce(to_char(created_date,'dd-mm-yyyy'),'') as created_date,is_active,doc_download_name,ctc,doc_mail_status,candidate_id,cost_to_employee_id,created_by_id,job_title_id,modified_by_id,offer_release_id from ta_offer_info where candidate_id = {0} and job_title_id = {1} and offer_release_id=655
            """.format(int(job_openings_login_details[0]['id']),int(job_openings_login_details[0]['job_opening_id'])))
            interview_offer_details = q.dictfetchall(cur)
            json_data['interview_offer_details'] = interview_offer_details
            print"FFFFFFFFFFF",json_data
            return HttpResponse(json.dumps(json_data),status=200)
        else:
            ...
            if job_fields:
                ...
                if is_valid != None and is_valid : 
                    ...
                    if job_fetch_details:
                        ...
                        str(job_fields[0]['country']),str(job_fields[0]['state']),str(job_fields[0]['gender']),str(job_fields[0]['job_id']),str(job_fields[0]['pin']),str(job_fields[0]['password']),str(modified_date),str(job_fields[0]['email']),str(job_fields[0]['mobile'])))
                        ...
                        str(job_fields[0]['country']),str(job_fields[0]['state']),str(job_fields[0]['gender']),str(job_fields[0]['job_id']),str(job_fields[0]['pin']),str(job_fields[0]['password']),str(created_date),str(modified_date),))
    ...
    try:
            ...
            if job_details:
                ...
                if job_attach_details:
                    ...
                    attachment_path = '/home/nextaps/.green_hcms/uploads/Interview/'+attachment
                    ...
                    content = '''
                                ...
                                <p>Please login to HCMS https://mygreen.green.com.pg to review the received application.</p>
                                <p>Regards,</p> 
                                <p>GREEN Admin</p> 
                                <p>GREEN Telemed.</p> 
...
def feedback(request):
    ...
    try:
        ...
        print"FEEEEEEE",feedback_fields
	...
	print"FEEEEEEE",feedback_fields
        ...
        if is_valid != None and is_valid : 
            cur.execute("select id from feedback where feedback_email='{0}' and feed_mobile='{1}'".format(str(feedback_fields[0]['feedback_email']),str(feedback_fields[0]['feed_mobile'])))
            job_openings_details = q.dictfetchall(cur)
            if job_openings_details:
                ...
                cur.execute("insert into feedback (feed_name,feed_mobile,feed_type,feedback_email,feed_role,rating,comments) values (%s,%s,%s,%s,%s,%s,%s)",
                            (str(feedback_fields[0]['feed_name']),str(feedback_fields[0]['feed_mobile']),int(feedback_fields[0]['feed_type']),
                ...
                mobile = str(feedback_fields[0]['feed_mobile'])      
