...
class HRMSPaymentAdvice(TemplateView):
	...
	cur.execute(q.fetch_hcms_query(config.payroll_management, config.hrms_select_banking_payer_info));
        payer_info_data = q.dictfetchall(cur)  	
        if payer_info_data:
            payer_info_data = payer_info_data
        else:
            payer_info_data = []  
        context[config.banking_payer_info] = payer_info_data #Loading Employee Data
	print"--------------------------------------",context
...
def HRMSCreatePaymentAdvice(request): # employee details create function
    ...
    try:
		...
		print"RRRRRRRRR",data['payment_adivces'][0]['payment_mode']
                if payment_id == '0':
			if data['payment_adivces'][0]['payment_mode'] != "8":
			    status = PaymentAdvices(company_id_id=data['payment_adivces'][0]['company_id_id'], employee_id_id=data['payment_adivces'][0]['employee_id_id'],payment_mode_id=data['payment_adivces'][0]['payment_mode'], account_holder_name=data['payment_adivces'][0]['account_holder_name'],
                                                  ...
                                                  uan_number=data['payment_adivces'][0]['uan_number'],ifsc_code=data['payment_adivces'][0]['ifsc_code'],
                            ...
                            status = PaymentAdvices(company_id_id=data['payment_adivces'][0]['company_id_id'], employee_id_id=data['payment_adivces'][0]['employee_id_id'],payment_mode_id=data['payment_adivces'][0]['payment_mode'], account_holder_name=data['payment_adivces'][0]['account_holder_name'],
                                                    organization_unit_id = data['payment_adivces'][0]['organization_unit_id'],department_id = data['payment_adivces'][0]['department_id'],banking_payer_id=data['payment_adivces'][0]['banking_payer_id'],
                                                  bank_name_id=data['payment_adivces'][0]['bank_name_id'],branch_name=data['payment_adivces'][0]['branch_name'],account_no=data['payment_adivces'][0]['account_no'],
                                                  uan_number=data['payment_adivces'][0]['uan_number'],ifsc_code=data['payment_adivces'][0]['ifsc_code'],
                                                  pan_number=data['payment_adivces'][0]['pan_number'],pf_number=data['payment_adivces'][0]['pf_number'],pf_applicable=data['payment_adivces'][0]['pf_applicable'],is_active="True",created_by_id=uid)
                        status.save()
