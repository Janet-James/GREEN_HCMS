        ...
        if data_value :
                ...
                if contract_tbl_id == '0': 
                        ...
                        for v in employee_list_value:
                            ...
                            if employee_res:
                                ...
                                status = SalaryContract(company_id_id = data['salary_contract_data'][0]['company_id_id'],org_unit_id_id = data['salary_contract_data'][0]['organization_unit_id_id'], department_id_id = data['salary_contract_data'][0]['department_id_id'],
                                                    ...
                                                    contract_effective_from_date = data['salary_contract_data'][0]['contract_effective_from_date'],contract_effective_to_date = data['salary_contract_data'][0]['contract_effective_to_date'],variable=data['salary_contract_data'][0]['variable'],grade= data['salary_contract_data'][0]['salary_grade'],nationality_check= data['salary_contract_data'][0]['national_check'],is_active ="True",created_by_id=uid)
                                status.save()
                    ...
                    status = SalaryContract.objects.filter(id=contract_tbl_id).update(company_id_id = data['salary_contract_data'][0]['company_id_id'],org_unit_id_id = data['salary_contract_data'][0]['organization_unit_id_id'], department_id_id = data['salary_contract_data'][0]['department_id_id'],
                                                  ...
                                                  job_title = data['salary_contract_data'][0]['job_title'],salary_structure_id_id = data['salary_contract_data'][0]['salary_structure_id_id'],base_amount = data['salary_contract_data'][0]['base_amount'],base_currency_id = data['salary_contract_data'][0]['base_currency_id']
                                                  ,contract_effective_from_date = data['salary_contract_data'][0]['contract_effective_from_date'],contract_effective_to_date = data['salary_contract_data'][0]['contract_effective_to_date'],variable= data['salary_contract_data'][0]['variable'],grade= data['salary_contract_data'][0]['salary_grade'],nationality_check= data['salary_contract_data'][0]['national_check'],is_active="True",modified_by_id=uid)
                    if contract_tbl_id:    
                        if update_data:                
...
def HRMSSalaryContractTblDispaly(request):
    try:
        ...
        if request.method == config.request_post:
            ...
            if table_data:
                for i in table_data:
                    #ekm_data = ekm_process('fb4a04a8b086e06c168d9c1d1fbd9aa9','string',i['base_amount'],'decrypt')
            ...
            if row_click_id:
                ...
                if salary_contract_table_row_click:
                    ...
                    if salary_contract_tbl_click:
                        #EKM// ekm_data = ekm_process('fb4a04a8b086e06c168d9c1d1fbd9aa9','string',salary_contract_tbl_click['datas'][0]['base_amount'],'decrypt')
...
def HCMSSalaryRating(request):
	...
	eff_from_date=datetime.datetime.strptime(from_date, '%d-%m-%Y').strftime('%Y-%m-%d')
	eff_to_date=datetime.datetime.strptime(from_date, '%d-%m-%Y').strftime('%Y-%m-%d')
	cur.execute("""select variable from hcms_pm_strategic_objectives where 
	employee_id=%s AND ((start_date >= %s AND end_date <= %s) 
	OR (end_date >= %s AND start_date <= %s))""",(employee_id,eff_from_date,eff_to_date,eff_from_date,eff_to_date,))
	variable_data = q.dictfetchall(cur)
	result_data['variable']=variable_data
    ...
    print"RESUUUUUUUUUUUUUUUUUUUUUUUUUU",result_data     
