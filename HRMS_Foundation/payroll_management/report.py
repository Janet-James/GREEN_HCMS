...
def HRMSReport(request): # employee details create function
    ...
    print"TRUEEEEEEEEE",from_date,to_date,to_date.split('-')[1]
...
#                                 hr_hour_val = datetime.datetime.strptime('08:00:00', '%H:%M:%S')
		    ...
		    ctc_amount = float(base_amount[0]['base_amount'])
               ...
               if ctc_amount and working_days and worked_day:
                	...
                	if working_days > 0:
                    		...
                    		one_day_salary = 0.00
                    	final_data_value['per_salary']=one_day_salary
                	if worked_day > 0:
			...
			if employee_grade == 2 or employee_grade == 1:
				...
				if working_days > 0:
					...
					one_hour_salary = 0.00
                		final_data_value['per_salary']=one_hour_salary
				if total_hours:
		...
		team.name as team_name,sc.job_title as role_title,to_char(emp.date_of_joining,'DD/MM/YYYY') as date_of_joining,hr_payment_advices.account_no,
		hr_payment_advices.ifsc_code,refitems_name as bank_name,hr_payment_advices.branch_name from employee_info emp left join team_details_info team on team.id = emp.team_name_id 
		left join hcms_ti_role_details tirole on tirole.id = role_id_id left join hr_payment_advices on hr_payment_advices.employee_id_id = emp.id and hr_payment_advices.is_active 
		left join reference_items on reference_items.id = hr_payment_advices.bank_name_id 
		left join hr_salary_contract sc on emp.id=sc.employee_id_id
...
#                         final_data_value['pvr_value'] = total_pvr_value 
               ...
               if structure_id and employee_id:
	               ...
	               print"OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO",earned_basic_amount_value,employee_nationality,type(employee_nationality),employee_grade
	       	       if employee_nationality == False:
						...
						#if employee_grade == 1:
							#print"Professionallllllllllllllll",worked_day
							#if working_days > 0:
                    					#	one_day_salary = ctc_amount / float(11)
                					#else:
                    					#	one_day_salary = 0.00
		   ...
		   print"DDDDD",variable_amount
		   if variable_amount:
                   	if int(variable_amount) > 0:
                        	#ctc_amount = 0
                        	print"EEEEEEEEee",variable_amount,ctc_amount
...
#                                                 t['assignment_value'] = 0
               ...
               print"EEEEEEEEEEEEEEEEEEEEEE",total_earnings_pdf,float(arrear_amount_val),variable_amount,total_lvr_amount,total_pvr_value,earned_special_allowance
...
# font
    ...
    # sh.write(0, 6, 'Bank Code',style=style)
    # sh.write(0, 7, 'Bank Name',style=style)
    sh.write(0, 6, 'Actual Hours Worked',style=style)
    #sh.write(0, 5, 'PF No',style=style)
    #sh.write(0, 9, 'Total Worked Days',style=style)
    sh.write(0, 7, 'Absent in Hours',style=style)
    # sh.write(0, 11, 'Non Compliance Days',style=style)
    sh.write(0, 8, 'Rate Per Hour',style=style)
    sh.write(0, 9, 'CTC',style=style)
    sh.write(0, 10,'Basic',style=style)
    ...
    sh.write(0, 11,'Arrear',style=style)
    sh.write(0, 12,'Variable Return',style=style)
    #sh.write(0, 24,'Leadership Value Rerturn',style=style)
    #sh.write(0, 25,'Product Value Rerturn',style=style)
    sh.write(0, 13, 'EARNED CTC',style=style)
    sh.write(0, 14,'Professional Tax',style=style)
    ...
    sh.write(0, 15, 'TOTAL ALLOWANCE',style=style)
    sh.write(0, 16, 'TOTAL DEDUCTION',style=style)
    sh.write(0, 17, 'NET SALARY',style=style)
   ...
   # earning_list = ['Basic','Conveyance','HRA','Meal Card','Leave Travel Allowance','Medical Allowance','Telephone Allowance']
	...
	print"PAYYYYYYYYYYYY",dict_value['exp_details']
        if len(dict_value['exp_details']) > 0:
            ...
            if j['employee_data_val']:
                for l in j['employee_data_val']:
                    ...
                    sh.write(n+line, 6, j['total_hours'])
                    #sh.write(n+line, 5, l['provident_fund_no'])
            # if j['working_days']:        
                # sh.write(n+line, 9, j['worked_day'])
            lop= 88 - int(j['total_hours'])
            if j['lop_days']:    
                sh.write(n+line, 7, lop)
            else:
                sh.write(n+line, 7, lop)    
            # if j['lop_hr_count']:    
            ...
            # else:
            if j['per_salary']:    
                sh.write(n+line, 8, str(round(j['per_salary'], 2)))
            if j.get('report_ctc'):
                sh.write(n+line,9, j['report_ctc'],style=style) 
                ...
                for z,m in enumerate(earning_list):
                    ...
                    for o in j['emp_actual_earning_list']:
                         if o['salary_rule_name'] == m:
                             value_match = 1
                             sh.write(n+line,10+z,o['assignment_value'])
                             earning_value += o['assignment_value']
			     tot_earning = earning_value
                             break;
                    if not value_match:
                        sh.write(n+line,10+z,0)
            if j.get('special_allowance'):
                ...
                tot_earning = earning_value + arrear_amount

            if j['arrear_amount'] > 0:   
                sh.write(n+line,11,j['arrear_amount'])  
            else:
                sh.write(n+line,11,0,style=style)
                      
            if j.get('variable_amount'):
                sh.write(n+line,12, j['variable_amount'])
            ...
            if j.get('pvr_value'):
                ...
                total_earnings = 0.0
                total_earnings =  tot_earning       
            sh.write(n+line,13,total_earnings,style=style)             
                ...
                for x,y in enumerate(deduction_list):
                    ...
                    for p in j['emp_actual_deduction_list']:
                         if p['salary_rule_name'] == y:
                             dect_match = 1
                             sh.write(n+line,14+x,p['assignment_value'])
                             deduction_val += p['assignment_value']
                             break;
                    if not dect_match:
                        sh.write(n+line,14+x,0)
	    ...
	    if j['allowance']:
		sh.write(n+line,15,j['allowance'],style=style)
		tot_earning =tot_earning+j['allowance']
            sh.write(n+line,16,tot_deduction_amount,style=style) 
            ...
            sh.write(n+line, 17,net_salary ,style=style)
            #if j.get('slab_name'):
