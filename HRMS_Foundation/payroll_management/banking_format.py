...
def HCMSSelectReportEmployee(request):
    ...
    if organization_data:
            ...
            organization_data = []  
    print"FFFFFFFFF",organization_data
    result_data['employee_val'] = employee_id #Loading Employee Data
    result_data['organization_val'] = organization_data
    return HttpResponse(json.dumps(result_data))

@csrf_exempt
def HCMSOrgBasedEmployee(request): # employee details create function
    ''' 
    ...
    '''
    #try:
    print '$$$$$$$$$$$$$$$$$$44',request.POST.get('org_id_list')
    cur = connection.cursor() 
    json_data = {}
    org_id_list = json.loads(request.POST.get('org_id_list'))
    if org_id_list:
    	org_id=tuple(org_id_list)
    	cur.execute("""Select id from employee_info where org_id_id in %s and is_active """,(org_id,))
    	employee_ids= q.dictfetchall(cur)
    	if employee_ids:
        	id = [x['id'] for x in employee_ids]
		json_data['employee_ids'] = id
    	else:
		json_data['employee_ids'] = []
    return HttpResponse(json.dumps(json_data))
    
@csrf_exempt
def HCMSBankingFormat(request): # employee details create function
    ''' 
    26-May-2018 VJY To HRMS Create Salary Rule function. 
    @param request: Request Object
    @type request : Object
    ...
    '''
    #try:
    print '$$$$$$$$$$$$$$$$$$44',request.POST.get('from_date')
    cur = connection.cursor() 
    json_data = {}
    data_value = request.POST.get('id')   
    from_date = request.POST.get('from_date')
    to_date = request.POST.get('to_date')
    payer_financial_institution =request.POST.get('payer_financial_institution')
    payer_name = request.POST.get('payer_name')
    payer_client_no = request.POST.get('payer_client_no')
    payment_type = request.POST.get('payment_type')
    payment_description = request.POST.get('payment_description')

    print"TRUEEEEEEEEE",from_date,to_date,payment_description
    split_mon_from = from_date.split('-')[1]
    pay_month_from = int(strptime(split_mon_from,'%b').tm_mon)
    print"RRRRR",pay_month_from 
    split_mon_to = to_date.split('-')[1]
    pay_month_to = int(strptime(split_mon_to,'%b').tm_mon)
    split_year= int(from_date.split('-')[2] )
    split_date = int(from_date.split('-')[0] )
    period_from = datetime.date(split_year, pay_month_from, int(from_date.split('-')[0]))
    print"PERRRRRRR FROMMMMMMM",period_from
    period_to = datetime.date(int(to_date.split('-')[2]), pay_month_to, int(to_date.split('-')[0]))
...
#                                 hr_hour_val = datetime.datetime.strptime('08:00:00', '%H:%M:%S')
        ...
        #                         check_in_time = datetime.datetime.strptime(i['check_in_time'], '%H:%M:%S')
               ...
               cur.execute("""select salary_structure_id_id,nationality_check, grade from hr_salary_contract  where employee_id_id = %s and (contract_effective_from_date,contract_effective_to_date) overlaps (%s,%s) and is_active """,(employee_id,period_from,period_to,))
		...
		where employee_id_id = {0} and is_active and check_in :: date >= '{1}'
	       ...
	       final_data_value['total_hours'] = total_hours
	       worked_day = float(total_hours/8)

               cur.execute("SELECT base_amount, variable from hr_salary_contract where  employee_id_id=%s and is_active=True", (employee_id,))
               base_amount = q.dictfetchall(cur)
	       variable_amount = base_amount[0]['variable']
	       if variable_amount is None:
			variable_amount=0
               if base_amount:
                    #ctc_amount = float(ekm_process('fb4a04a8b086e06c168d9c1d1fbd9aa9','string',base_amount[0]['base_amount'],'decrypt'))#basic_amount[0]['base_amount']  
			...
			if employee_grade == 2 or employee_grade == 1:
               ...
               cur.execute("""select COALESCE(concat(upper(emp.name), ' ',upper(emp.last_name)),'') as employee_name,emp.employee_id as employee_id,
                    ...
                    hr_payment_advices.account_no,hr_payment_advices.ifsc_code as transaction_code,hr_payment_advices.branch_name as branch_code,hr_payment_advices.account_type,refitems_name as bank_code,hr_payment_advices.branch_name,
					bp.payer_bank_code,bp.payer_branch_code,bp.payer_account_type,bp.payer_account_no,hr_payment_advices.account_holder_name
				...
				left join payroll_banking_payer bp on hr_payment_advices.banking_payer_id = bp.id 
...
#                         final_data_value['pvr_value'] = total_pvr_value 
               ...
               if structure_id and employee_id:
	               ...
	               print"OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO",earned_basic_amount_value,employee_nationality,type(employee_nationality),employee_grade
	       	       if employee_nationality == False:
						print"EEEEEEEEEEEEEE",false
                				if working_days > 0:
                    					one_day_salary = ctc_amount / float(working_days)
                				else:
                    					one_day_salary = 0.00
						...
						if employee_grade == 2:
							...
							if total_hours:
								...
								print"llllllllllllllllllllll", one_hour_salary,total_hours
                    						...
                    						earned_basic_amount_value = 0.00 
		       print"IIIIIIIIIIIIIIIIIIIIIIIIIIIII",earned_basic_amount_value,employee_nationality
               ...
               if structure_id and employee_id and worked_ctc_amount:
                   if int(variable_amount) > 0:
                        #ctc_amount = 0
                        esi_ctc_amount =  float(variable_amount) + ctc_amount
                   else:  
...
#                                         m['assignment_value'] = 0
		       ...
		       print"FIXED EARNINGGGGGGGGGGGGGGGGGGGGGG",Fixed_Earnings_list
			   		...
			   		if s['salary_rule_name']=='Basic':
                					...
                					if worked_day > 0:
                        				...
                        				if employee_grade == 2 or employee_grade == 1:
                            					...
                            					if total_hours:
                                					...
                                					s['assignment_value'] = 0.00
                                					print"ASSIGNMENT_VALUE-------------------------------",s,s['assignment_value']
            					...
            					total_earnings_pdf = total_earnings_pdf + earnings_amount_value
            					actual_earning_amount.append(s)
						allowance_data= allowance_data + s['assignment_value']
		       print"TOTAL EARNING--------",total_earnings_pdf,allowance_data
...
#                                                 t['assignment_value'] = 0
               ...
               #print"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$",final_data_value,allowance_data
               #final_data_value['total_lvr_value'] = round(total_lvr_value)
               #final_data_value['total_pvr_value'] = 0
               final_data_value['allowance']=allowance_data
	       final_data_value['variable_amount']=float(variable_amount)
               total_yr_earnings = total_earnings_pdf + float(arrear_amount_val) + float(variable_amount) + total_lvr_amount + total_pvr_value + earned_special_allowance
...
#                                    current_date = datetime.datetime.today().strftime("%d/%m/%Y")
            ...
            if result_data:
                result = csv_generate(request,result_data,worked_day,working_days,lop_days,payer_financial_institution,payer_name,payer_client_no,payment_type,payment_description)
...
def excel_generate(request,dict_value,worked_day,working_days,lop,payer_financial_institution,payer_name,payer_client_no,payment_type,payment_description):
...
# font
	...
	print"PAYYYYYYYYYYYY",dict_value['exp_details']
...
def csv_generate(request,dict_value,worked_day,working_days,lop,payer_financial_institution,payer_name,payer_client_no,payment_type,payment_description):	
    cr = connection.cursor()
    report_date = datetime.datetime.now()
    file_date = report_date.strftime("%Y-%m-%d %H:%m:%S.%f")[:-3]
    publish_date = report_date.strftime("%Y%m%d")
    print"DFFFFFFFFFF",file_date
    uid = request.user.id
    if not uid:
        uid = 1
    header = [payer_financial_institution, payer_name, payer_client_no, payment_type, publish_date,None,None,None,None,None,None,None]
    data = [
    ...
    earning_list = ['Basic']
    deduction_list = ['Professional Tax']
    employer_deduction = ['ESI','Provident Fund']
    payment_overall_data=[]
    overall_data=[]
    for j in dict_value['exp_details']:
        ...
        if len(dict_value['exp_details']) > 0:
            ...
            tot_deduction = 0.00 
            total_earnings = 0.00
            total_earning_variable = 0.00
            lvr_amt = 0.00
            pvr_amt = 0.00
            earning_value = 0.00
            deduction_val = 0.00
            deduction_val_employer = 0.00
            total_deduction_val = 0.00
            total_tds_amount = 0.00
            tot_deduction_amount = 0.0
            lvr_total_earnings = 0.00
            arrear_amount = 0.00
            if j['tds_amount']:
                total_tds_amount = float(j['tds_amount'])
            ...
            if j.get('emp_actual_earning_list'):
                for z,m in enumerate(earning_list):
			     ...
			     tot_earning = earning_value
                             break;
	    print"total_earning",tot_earning
            if j.get('special_allowance'):
                special_amt = float(j['special_allowance'])
	    if j['arrear_amount']:
                arrear_amount = float(j['arrear_amount'])
		tot_earning = tot_earning + arrear_amount
		print"Arrear added",tot_earning,arrear_amount

	    if j.get('allowance'):
		tot_earning = tot_earning + j.get('allowance')   
	    	print"allowance added",tot_earning,j.get('allowance') 
	    print"SSSSSSSSSS",  j               
            if j.get('variable_amount'):
                variable_amt = float(j['variable_amount'])
                if variable_amt !=0.0 :
            ...
            if  deduction_val > deduction_val_employer:
                ...
                tot_deduction =  deduction_val_employer +  deduction_val     
	    print"DEDEDDDDDDDDDDDD",tot_deduction_amount   
            tot_deduction_amount = tot_deduction +  total_tds_amount
            net_salary = tot_earning - tot_deduction_amount
            amount_words = v.number_to_words(int(net_salary))
            netsalary_letter = amount_words.title()
	    ...
	    if j['employee_data_val']:
                for l in j['employee_data_val']:
                    ...
                    overall_data.append(str(l['bank_code']))
                    overall_data.append(l['branch_code'])
                    overall_data.append(l['account_type'])
                    overall_data.append(l['account_no'])
                    overall_data.append(l['transaction_code']) 
		    overall_data.append(net_salary)
		    overall_data.append(l['account_holder_name'])
		    overall_data.append(payment_description)	            
	    	    overall_data.append(str(l['payer_bank_code']))
		    overall_data.append(l['payer_branch_code'])
		    overall_data.append(str(l['payer_account_type']))
		    overall_data.append(l['payer_account_no'])
	    print"OVERALLLLLLLLLL",overall_data 
    
    	    payment_overall_data.append(overall_data)
    print"OVERALLLLLLLLLLLLLLLLLLL",payment_overall_data
    file_name_val = "Payroll_bank_format"+"_"+file_date
    file_name_replace = file_name_val.replace(' ', '_')
    status = Report(file_name = file_name_replace, file_path = setting.MEDIA_ROOT+"payroll_bank_format/",file_type = 'csv', 
                   file_full_path = setting.MEDIA_ROOT+'payroll_bank_format/'+file_name_replace+".csv",created_by_id=uid, modified_by_id=uid)
    status.save()

    files=setting.MEDIA_ROOT+'payroll_bank_format/'+file_name_replace+'.csv'
    print"FILEEEEEEEE",files
    with open(files,'w') as f:
   		writer = csv.writer(f)
    		...
    		# write multiple rows
    		writer.writerows(payment_overall_data) 
    #book.save(setting.MEDIA_ROOT+'payroll_bank_format/'+file_name_replace+".csv")
    cr.execute("select file_name,file_full_path from report where id = %s",(status.id,));
    report_data = q.dictfetchall(cr)
    print"REPORT DAATA",report_data
    result = {}
    result['file_data'] = report_data
    result['success_data'] = 'Success'
    return result  
