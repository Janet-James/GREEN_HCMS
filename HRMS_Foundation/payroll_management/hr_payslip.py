...
def PayslipWorkedDays(request):
    ...
    employee_nationality = post.get('employee_nationality')
    employee_grade = post.get('employee_grade')
    total_hours=0
    absent_hours=0
    print"EEEEEEEEEEEEEEEEEEEE",employee_nationality,employee_grade
    lop_count = 0
    if employee_nationality == 'true':
	print"TRUEEEEEEEEE",period_from_val,period_to_val,period_from_val.split('-')[1]
	split_mon_from = period_from_val.split('-')[1]
	pay_month_from = int(strptime(split_mon_from,'%b').tm_mon)
	split_mon_to = period_to_val.split('-')[1]
	pay_month_to = int(strptime(split_mon_to,'%b').tm_mon)
	split_year= int(period_from_val.split('-')[2] )
	split_date = int(period_from_val.split('-')[0] )
	period_from = datetime.date(split_year, pay_month_from, int(period_from_val.split('-')[0]))
	print"PERRRRRRR FROMMMMMMM",period_from
    	period_to = datetime.date(int(period_to_val.split('-')[2]), pay_month_to, int(period_to_val.split('-')[0]))
	print"PERRRRRRR FROMMMMMMM",period_to
    else:	
    	if period_from_val:
	...
	print"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP",period_from,period_to
    non_atten_val = 0
    new_join_date = ''
...
((from_date::date>=%s and from_date::date<=%s)
    ...
    if total_working_days:
        ...
        total_working_days = ''
    cur.execute("""SELECT COUNT(check_in) as worked_days, SUM(EXTRACT(EPOCH FROM (check_out - check_in ))/3600)::integer as worked_hours, 
		SUM(CASE when (To_Char("check_in", 'DAY') not ilike '%SUNDAY%' OR To_Char("check_in", 'DAY') not ilike '%SATURDAY%') 
		AND (EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer >9 then ((EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer) - 8 
		WHEN To_Char("check_in", 'DAY') ilike '%SATURDAY%' AND (EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer >6 
		then ((EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer) - 8 
		WHEN To_Char("check_in", 'DAY') ilike '%SUNDAY%' 
		then ((EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer)		
		end)as total_overtime_hours,
		SUM(CASE when (To_Char("check_in", 'DAY') not ilike '%SUNDAY%' OR To_Char("check_in", 'DAY') not ilike '%SATURDAY%') 
		AND (EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer >9 then ((EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer) - 8 end) as week_day_overtime,
		SUM(CASE when To_Char("check_in", 'DAY') ilike '%SATURDAY%' AND (EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer >6  
		then ((EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer) end) as saturday_overtime,
		SUM(CASE when To_Char("check_in", 'DAY') ilike '%SUNDAY%'  
		then ((EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer) end) as sunday_overtime,
		SUM(EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer as total_hours from attendance_info 
		where employee_id_id = {0} and is_active and check_in :: date >= '{1}'
                                and check_in :: date <= '{2}'""".format(employee_id,start_date,end_date))
    total_hours_data = q.dictfetchall(cur)
    print 'total_hours_data------------',total_hours_data
    if total_hours_data[0]['total_hours']:
	total_hours = total_hours_data[0]['total_hours']
	if total_hours < 88:
		absent_hours= 88 - total_hours
	else:
		absent_hours = 0
    result['total_hours']=total_hours
    result['absent_hours']=absent_hours
    ...
    #print"DDDDDDDDDDDDDDDDDD",daygenerator
...
#                     hrpdts.check_in::date >= %s and hrpdts.check_in::date <= %s order by check_in""",(employee_id,period_from,period_to,))  
                ...
                if joining_date:
                    ...
                    ## attendance leave function         
                        ...
                        lop_count = 0
			print"WORKINGGGGGGGGGGGG DATAAAAAAAA--------",emp_working_data
...
#                                 hr_hour_val = datetime.datetime.strptime('08:00:00', '%H:%M:%S')
                                ...
                                if grace_shift_name != 'No Conflict':
                                    ...
                                    lop_count = 0
                                print"=====================================",lop_count,i['check_in'],i['check_in_time']     
                    else:
        ...
        if no_attendance_employee or no_exist_employee:                
             result = {'leave_lop': 0, 'lop_count': 0, 'total_working_days': 0, 'worked_days': 0, 'total_hours': 0,'absent_hours':0}
...
def PayslipStructureSalaryRule(request):
    ...
    #try:#Try Block
        ...
        if cur: 
            ...
            employee_nationality = post.get('employee_nationality')
    	    employee_grade = post.get('employee_grade')
            print"EEEEEEEEEEEEEEEEEEEE",employee_nationality,employee_grade,type(employee_grade)
            if employee_nationality == 'true':
		print"TRUEEEEEEEEE",period_from_val,period_to_val,period_from_val.split('-')[1]
		split_mon_from = period_from_val.split('-')[1]
		pay_month_from = int(strptime(split_mon_from,'%b').tm_mon)
	    ...
	    total_hours = 0
            ...
            if payslip_emp_fixed_monthly_ctc_amount_query:
                cur.execute(payslip_emp_fixed_monthly_ctc_amount_query, (employee_id,))
		   ...
		   okr_variable = base_amount[0]['variable']
                   #ctc_amount = ctc_amount + float(arrear_amount_val)
            #print"==========================ctc_amount",ctc_amount ,working_days,worked_day,okr_variable
		...
		where employee_id_id = {0} and is_active and check_in :: date >= '{1}'
            ...
            print 'total_hours_data------------',total_hours_data
            if total_hours_data[0]['total_hours']:
		#total_hours = total_hours_data[0]['total_hours'] - total_hours_data[0]['total_overtime_hours']
		total_hours = total_hours_data[0]['total_hours']
            week_day_overtime = total_hours_data[0]['week_day_overtime']
            sunday_overtime = total_hours_data[0]['sunday_overtime']
            saturday_overtime =total_hours_data[0]['saturday_overtime']
	    worked_day = total_hours_data[0]['worked_days']
            if ctc_amount and working_days and worked_day:
		if employee_nationality == 'false':
                	if working_days > 0:
			...
			#if employee_grade == '1':
			#	print"Professionallllllllllllllll"
			#	if working_days > 0:
                    	#		one_day_salary = ctc_amount / float(11)
                	#	else:
                    	#		one_day_salary = 0.00
                	#	if worked_day > 0:
                    	#		worked_ctc_amount = one_day_salary * float(worked_day)
			#	else:
                    	#		worked_ctc_amount = 0.00
			if employee_grade == '1' or employee_grade == '2':
				print"Laborrrrrrrrrrrrrrrrrrrrrrrrr",total_hours
				if working_days > 0:
					one_hour_salary = ctc_amount / float(88)
				else:
					one_hour_salary = 0.00
				
                                #if week_day_overtime:
                                 #       total_hours=total_hours+week_day_overtime
				#if sunday_overtime:
                                 #       total_hours=total_hours+(sunday_overtime*2)
                                #if saturday_overtime:
                                 #       total_hours=total_hours+(saturday_overtime*1.5)


				if total_hours:
					worked_ctc_amount = one_hour_salary * float(total_hours)
				print"llllllllllllllllllllll", one_hour_salary,total_hours
            if ctc_amount:
                result_data['fixed_ctc_amount'] = ctc_amount
	    print"CTC---------------->",ctc_amount,worked_day
	    if okr_variable:
		result_data['okr_variable'] = okr_variable
...
#                     product_type_details = q.dictfetchall(cur)
            ...
            if structure_id and employee_id:
                ...
                if payslip_salary_rule_basic_name_query:
                    ...
                    if result_basic:
                        ...
                        fixed_basic_amount_value = ''
	            print"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$",fixed_basic_amount_value
				    ...
				    print"^^^^^^^^^^^^^^^^^^^^^^^^^^^^^///",earned_basic_amount_value,float(working_days),float(worked_day)
					...
					if employee_nationality == 'false':
                				if working_days > 0:
                    					...
                    					one_day_salary = 0.00
                				if worked_day > 0:
                    					earned_basic_amount_value = one_day_salary * float(worked_day)
                				else:
                    					earned_basic_amount_value = 0.00
					else:
						#if employee_grade == '1':
						#	print"Professionallllllllllllllll"
						#	if working_days > 0:
						...
						#	else:
                    				#		earned_basic_amount_value = 0.00
						if employee_grade == '2' or employee_grade == '1':
							print"Laborrrrrrrrrrrrrrrrrrrrrrrrr",total_hours,worked_day
							if working_days > 0:
								one_hour_salary = ctc_amount / float(88)
							else:
								one_hour_salary = 0.00
							if total_hours:
								earned_basic_amount_value = one_hour_salary * float(total_hours)
								print"llllllllllllllllllllll", one_hour_salary,total_hours
							else:
                    						earned_basic_amount_value = 0.00
                        ...
                        earned_basic_amount_value = ''
            print"EARRRRRRRRRRRR----------------",earned_basic_amount_value  
            ...
            if structure_id and employee_id:
                ...
                list_value_earnings = fixed_earnings_calculation(result_data, structure_id, employee_id, list_value_earnings, ctc_amount, okr_variable, fixed_basic_amount_value, lop_days, earned_basic_amount_value)
            ...
            if structure_id and employee_id:
		print"CTCCCCCCCCCCC",ctc_amount,variable_amount,ctc_amount

                #if int(variable_amount) > 0:
                #ctc_amount = 0
                 #   ctc_amount = int(variable_amount) + ctc_amount
                #else:  
                 #   ctc_amount = 0 + ctc_amount
                print"CTCCCCCCCCCCC",ctc_amount
		...
		print"DEDUCTION________",list_value_deduction
           ...
           # result_data['employer_contribution_data'] = emp_data
            ...
            if Fixed_Earnings_list:
                for s in Fixed_Earnings_list:
                    print"ASSIGNMENT_VALUE1111111111----------------------",s,worked_day
                    if s['salary_rule_name'] =='Basic':
                        if employee_nationality == 'false':
                                        if working_days > 0:
                                                one_day_salary = ctc_amount / float(working_days)
                                        else:
                                                one_day_salary = 0.00
                                        if worked_day > 0:
                                                s['assignment_value'] = round(one_day_salary * float(worked_day))
                                        else:
                                                s['assignment_value'] = 0.00
                        else:
                                #if employee_grade == '1':
                                 #   print"Professionallllllllllllllll",ctc_amount
                                 #   if working_days > 0:
                                 #                   one_day_salary = ctc_amount / float(11)
                                 #   else:
                                 #                  one_day_salary = 0.00
                                 ...
                                 #                   s['assignment_value'] = 0.00
                                if employee_grade == '2' or employee_grade == '1':
                                    print"Laborrrrrrrrrrrrrrrrrrrrrrrrr"
                                    if working_days > 0:
                                        one_hour_salary = ctc_amount / float(88)
                                    else:
                                        one_hour_salary = 0.00
                                    if total_hours:
                                        s['assignment_value'] = round(one_hour_salary * float(total_hours))
                                        print"llllllllllllllllllllll", one_hour_salary,total_hours
                                    else:
                                                    s['assignment_value'] = 0.00

                                #s['assignment_value'] = round(float(s['assignment_value']) / float(working_days) * float(worked_day))
                        print"ASSIGNMENT_VALUE-------------------------------",s['assignment_value']
...
#                                         t['assignment_value'] = 0
	    ...
	    print"SPECIAL ALLLOWANCE COMMANDED",result_data
...
def fixed_earnings_calculation(result_data, structure_id, employee_id, list_value_earnings, ctc_amount, okr_variable, fixed_basic_amount_value, lop_days, earned_basic_amount_value):
    ''' 
        ...
        if cr: 
            ...
            if earnings_payslip_structure_salary_rule:
                ...
                for i in earnings_result:
                    ...
                    if contributor != 'Employer':
                        # For Common Function of Salary Rule Execution
                        list_value_earnings = common_salary_rule_function(result_data, i, val_dic, validation, contribution_id, condition_id, list_value_earnings, ctc_amount,okr_variable,fixed_basic_amount_value, lop_days, earned_basic_amount_value)
    except Exception as e:#Exception Block
        logger_obj.error(e)
        result = e    
...
def common_salary_rule_function(result_data, i, val_dic, validation, contribution_id, condition_id, list_value, ctc_amount, okr_variable, fixed_basic_amount_value, lop_days, earned_basic_amount_value):
    ''' 
        17-May-2018 VIJ To HR Payroll Payslip page loaded. And also check the user authentication
        @param request: Request Object
        @type request : Object
    ...
    try:
        ...
        if validation:
            validation_condition = ''
            # For CTC in Salary Rule Validation Code
            validation_condition = common_replace_code(result_data,i, validation_condition, validation, ctc_amount, okr_variable, fixed_basic_amount_value, lop_days, earned_basic_amount_value)
        # If No validation
        else:
            validation_condition = ''
        # If Validation Condition
            ...
            if i['cal_type_code'] == "NUMBR":
                ...
                if i['salary_rule_value_assignment_code']:
                    ...
                    if assignment_value:
                        # For Replacing the BASIC Code to as Basic Amount
                        # For CTC in Salary Rule Validation Code
                        assignment_value = common_replace_code(result_data,i, assignment_value, assignment_value, ctc_amount, okr_variable, fixed_basic_amount_value, lop_days, earned_basic_amount_value)
                        if "%" in assignment_value:  # For Find the % in Original String
...
def common_replace_code(result_data,i,validation_condition, validation, ctc_amount, okr_variable, fixed_basic_amount_value, lop_days, earned_basic_amount_value):
    ''' 
        17-May-2018 VIJ To HR Payroll Payslip page loaded. And also check the user authentication
        @param request: Request Object
        @type request : Object
...
def HRMSCreatePayslip(request): # employee details create function
    ...
    if request:
		...
		print"FROM----------",data['payslip_data'][0]['accountingperiod_from'],data['payslip_data'][0]['accountingperiod_to']
		period_from_val = data['payslip_data'][0]['accountingperiod_from']
		period_to_val = data['payslip_data'][0]['accountingperiod_to']
                employee_nationality = data['payslip_data'][0]['employee_nationality']
		if employee_nationality == 'true':
			print"TRUEEEEEEEEE",period_from_val,period_to_val,period_from_val.split('-')[1]
			split_mon_from = period_from_val.split('-')[1]
			pay_month_from = int(strptime(split_mon_from,'%b').tm_mon)
			...
			print"PERRRRRRR FROMMMMMMM",period_to
    			...
    			if period_from_val:
        			split_mon = period_from_val.split('-')[1]
    			if period_to_val:
        			split_year = int(period_from_val.split('-')[2] )
    			if split_mon:
        			pay_month = int(strptime(split_mon,'%b').tm_mon)
			...
			print"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP",period_from,period_to
                ...
                cur.execute("""select base_amount from hr_salary_contract  where employee_id_id = %s and is_active""",
                            (data['payslip_data'][0]['employee_id_id'],))
...
def PayslipPrint(request):
    #try:#Try Block
       ...
       if cr: 
           ...
           if select_payslip_data_record:
			   ...
			   print"DATEEEEEEEEEEEEEE",period_from,period_to
                           payslip_table_id = i['id']
                           result_rating = {}
                           total_earnings_pdf = 0.0
                           total_deductions_pdf = 0.0
                           employer_ded_pdf = 0
			   cr.execute("""select salary_structure_id_id,nationality_check, grade from hr_salary_contract  where employee_id_id = %s and (contract_effective_from_date,contract_effective_to_date) overlaps (%s,%s) and is_active """,(employee_id,period_from,period_to,))
               		   structure_data = q.dictfetchall(cr)
               		   if structure_data:
                    		structure_id = structure_data[0]['salary_structure_id_id']  # Get the Structure ID
	            		employee_nationality = structure_data[0]['nationality_check']
		    		employee_grade = structure_data[0]['grade']
               	           else:
                    		structure_id = ''
		    		...
		    		employee_grade = ''
                           ...
                           print"starrrrrrrrrrr",start_date,end_date
                           ...
                           if payslip_emp_fixed_monthly_ctc_amount_query:
                               ...
                               if basic_amount:
				   okr_variable=basic_amount[0]['variable']
				   ...
				   ctc_amount =float(basic_amount[0]['base_amount'])
                           if ctc_amount and working_days and worked_day:
                               ...
                               if worked_day > 0:
                                   ...
                                   worked_ctc_amount = 0.00
			   cr.execute("""SELECT COUNT(check_in),SUM(
				CASE when To_Char("check_in", 'DAY') ilike '%SUNDAY%' then (2*(EXTRACT(EPOCH FROM (check_out -check_in ))/3600))::integer
				WHEN To_Char("check_in", 'DAY') ilike '%SATURDAY%' AND (EXTRACT(EPOCH FROM (check_out -check_in ))/3600)::integer >4 then (1.5*(EXTRACT(EPOCH FROM (check_out -check_in ))/3600))::integer
				else (EXTRACT(EPOCH FROM (check_out - check_in ))/3600)::integer  end) as total_hours 
				from attendance_info 
				where employee_id_id = {0} and is_active and check_in :: date >= '{1}'
                                and check_in :: date <= '{2}'""".format(employee_id,start_date,end_date))
            		   total_hours_data = q.dictfetchall(cr)
            		   print 'total_hours_data------------',total_hours_data
            		   if total_hours_data[0]['total_hours']:
				total_hours = total_hours_data[0]['total_hours']
			   ...
			   if okr_variable:
				variable_amount=int(okr_variable)
    ...
    #                                 fig.set_figwidth(7)
                                    ...
                                    month_image_name = '/home/nextaps/.green_hcms/uploads/payslip_images/mon_rating_chart_'+str(employee_id)+'_'+file_date_replace+''
                                ...
                                #ax.plot(x, y)
                                    ...
                                    file_date_replace = file_date.replace(' ', '_')
                                    bvr_image_name = '/home/nextaps/.green_hcms/uploads/payslip_images/bvr_chart_'+str(employee_id)+'_'+file_date_replace+''
...
#                                             leadership_slab_value = float(leadership_value[0]['value_return']) * ctc_amount / 100    
                                    ...
                                    if total_lvr_amount > 0:
                                        ...
                                        file_date_replace = file_date.replace(' ', '_')
                                        lvr_image_name = '/home/nextaps/.green_hcms/uploads/payslip_images/lvr_rating_chart_'+str(employee_id)+'_'+file_date_replace+''
                               ...
                               if payslip_structure_salary_rule:
                                   ...
                                   for i in result:
                                       ...
                                       if contributor != 'Employer':
                                           ...
                                           if validation_condition:
                                               if i['cal_type_code'] == "NUMBR":
                                                   val_dic['assignment_value'] = float(i['salary_rule_value_assignment'])
                                               else:
                                                   # Check the Assignment Value 
                                                   if i['salary_rule_value_assignment_code']:
                                                       assignment_value = i['salary_rule_value_assignment_code']
...
#                                                                assignment_value = assignment_value.replace("ENCTC", str(ctc_amount))
                                                           ...
                                                           if "%" in assignment_value:  # For Find the % in Original String
                                                              ...
                                                              val_dic['assignment_value'] = float(eval(assignment_value))
                                                       else:
                                                           val_dic['assignment_value'] = ''
                                                   # Else the Assignment Value not Given
                                                   else:
...
#                                                                assignment_value = assignment_value.replace("ENCTC", str(ctc_amount))
                                   ...
                                   if Fixed_Earnings_list:
                                       for s in Fixed_Earnings_list:
					   print"fixed eatninnnnnnn",s['assignment_value']
					   ...
					   if employee_nationality == False:
                				if working_days > 0:
                    					one_day_salary = s['assignment_value'] / float(working_days)
                				else:
                    					one_day_salary = 0.00
                				if worked_day > 0:
                    					s['assignment_value'] = round(one_day_salary * float(worked_day))
						...
						#	print"Professionallllllllllllllll"
						#	if working_days > 0:
                    				#		one_day_salary = s['assignment_value'] / float(11)
                				#	else:
                    				#		one_day_salary = 0.00
                				#	if worked_day > 0:
                    				#		s['assignment_value'] = round(one_day_salary * float(worked_day))
						...
						if employee_grade == 2:
							print"Laborrrrrrrrrrrrrrrrrrrrrrrrr"
							if working_days > 0:
								one_hour_salary = s['assignment_value'] / float(88)
							else:
								one_hour_salary = 0.00
							if total_hours:
								s['assignment_value'] = round(one_hour_salary * float(total_hours))
								print"llllllllllllllllllllll", one_hour_salary,total_hours
                    						...
                    						s['assignment_value'] = 0.00
					   print"sdfsdfsdfsdf",s['assignment_value']
					   earnings_amount_value = s['assignment_value']
                                           total_earnings_pdf = total_earnings_pdf + earnings_amount_value
                                           actual_earning_amount.append(s)
					   print"llllllllllll",actual_earning_amount
                                   actual_deduction_amount = []
                                   if Fixed_Deduction_list:
                                        for a in Fixed_Deduction_list:
                                            cr.execute("""select esi_active from esi_value where esi_employee_id = %s and (esi_date_from,esi_date_to) overlaps (%s,%s) and is_active and esi_active""" , (employee_id,str(period_from),str(period_to),))
...
#                                                             t['assignment_value'] = 0
				   ...
				   final_data_value['special_allowance']=0
                                   if earned_special_allowance:
...
#                                    current_date = datetime.datetime.today().strftime("%d/%m/%Y")
                           ...
                           file_date_replace = file_date.replace(' ', '_')
                           year_date_image_name = '/home/nextaps/.green_hcms/uploads/payslip_images/year_date_chart_'+str(employee_id)+'_'+file_date_replace+''
                           plt.savefig(year_date_image_name, dpi=600, format='png', bbox_inches='tight')
                           plt.close()   
                           final_data_value['year_date_image_name'] = year_date_image_name
...
def demo_pdf(result_data):
    ...
    for a in result_data["exp_details"]:
        ...
        pdfkit.from_string(posts_html, '/home/nextaps/.green_hcms/uploads/payslip/temp_Payslip_of_'+a["employee_data_val"][0]["employee_name"]+'_'+str(empl_id)+ '_for_' +payslip_month+'_'+payslip_year+'.pdf', css=css,options=options) #with --page-size=Legal and --orientation=Landscape   
        ...
        if emp_details:
            ...
            pdffile = open(r'/home/nextaps/.green_hcms/uploads/payslip/temp_Payslip_of_'+a["employee_data_val"][0]["employee_name"]+'_'+str(empl_id)+ '_for_' +payslip_month+'_'+payslip_year+'.pdf', "rb")
            pdfReader = PyPDF2.PdfFileReader(pdffile)
            pdfWriter = PyPDF2.PdfFileWriter()
            for pageNum in range(pdfReader.numPages):
                pdfWriter.addPage(pdfReader.getPage(pageNum))
            pdfWriter.encrypt(emp_password.encode('utf-8'))
            resultPDF = open(r'/home/nextaps/.green_hcms/uploads/payslip/Payslip_of_'+a["employee_data_val"][0]["employee_name"]+'_'+str(empl_id)+ '_for_' +payslip_month+'_'+payslip_year+'.pdf', "wb")
            pdfWriter.write(resultPDF)
            resultPDF.close()
            os.remove('/home/nextaps/.green_hcms/uploads/payslip/temp_Payslip_of_'+a["employee_data_val"][0]["employee_name"]+'_'+str(empl_id)+ '_for_' +payslip_month+'_'+payslip_year+'.pdf')
...
def email_send(email,mail_subject,email_content,file_name):
    try:
        ...
        body = email_content
        if file_name:
              part = MIMEApplication(open(os.path.join("/home/nextaps/.green_hcms/uploads/payslip/")+file_name,"rb").read())
              part1=part
