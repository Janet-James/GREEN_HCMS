...
def Leave_Status_mobile(request):
    response = ''
    print"LLLLLLLLLLLLLLLLLLL",request.body      
    try:
        body = request.body
        try:
        ...
        if data:
            ...
            cur = connection.cursor()
            cur.execute(""" select a.id,a.refitems_desc as leave_type,a.description as reason,a.from_date,a.to_date,round( CAST(float8 (a.number_of_days::text) as numeric), 1)as number_of_days,a.state as leave_status,
...
def organization_details_mobile(request):
    try:
        body=request.body
        print"PIIIIIIIIIIIIIIIIIII",body
        try:
            data=json.loads(body)
            data=ast.literal_eval(data) 
        except:   
