...
var payslip_id = ''
	...
	var payslip_data = [];
	var global_employee_nationality =false
	var global_employee_grade = 0
...
function payslip_create_function(){
	...
	payslip_form_value['employee_nationality'] =global_employee_nationality
...
function payslip_compute_function(){
	...
	employee_nationality=global_employee_nationality
	employee_grade=global_employee_grade
	console.log(new Date(period_to_date),new Date(period_from_date),new Date(period_to_date).toDateString() > new Date(period_from_date).toDateString())
	if(structure_id && employee_id && period_from && period_to){
		if((new Date(period_to_date) > new Date(period_from_date)) || (new Date(period_to_date) == new Date(period_from_date)) ){
			//$('.error_status').html('')
			//For Worked Days and Working Days and LOP Days Calculation
			worked_days_calculation(employee_id,period_from,period_to,employee_nationality,employee_grade)
			//For Salary Computation Functionality
			structure_salary_rule(employee_id,structure_id,worked_day,working_days,lop_days,period_from,period_to,employee_nationality,employee_grade)
...
function worked_days_calculation(employee_id,period_from,period_to,employee_nationality,employee_grade)
{
	$.ajax({
		type:"POST",
		url:'/hr_payslip_worked_days/',
		data:{"period_from":period_from,"period_to":period_to,'employee_id':employee_id,'employee_nationality':employee_nationality,'employee_grade':employee_grade,csrfmiddlewaretoken:$("input[name=csrfmiddlewaretoken]").val()},
		async:false,	
	}).done(function(json_data){
		data = JSON.parse(json_data);
		...
		document.getElementById('hr_total_hrs_worked').value = data.total_hours
		document.getElementById('hr_absent_hours').value = data.absent_hours
...
function structure_salary_rule(employee_id,structure_id,worked_day,working_days,lop_days,period_from,period_to,employee_nationality,employee_grade){
	tds_amount_value = 0
	$.ajax({
		type:"POST",
		...
		timeout : 10000,
		async:false,
		data:{'employee_id':employee_id,'structure_id':structure_id,'working_days':working_days,'worked_day':worked_day,'lop_days':lop_days,"period_from":period_from,"period_to":period_to,"employee_nationality":employee_nationality,"employee_grade":employee_grade,csrfmiddlewaretoken:$("input[name=csrfmiddlewaretoken]").val()},
	}).done(function(json_data){
		data=JSON.parse(json_data)
		var earning_list = []
		var deduction_list = []
...
$("#hr_payslip_employee_name").change(function(){
	...
	$.ajax({
		...
		console.log(data[i].grade,data[i].nationality_check)
                if(data[i].nationality_check == true)
		{	
			global_employee_nationality=data[i].nationality_check
			global_employee_grade=	data[i].grade
			$('.tab_payslip').DateTimePicker({
