...
$(document).ready(function(){
	...
	$("#div_payment_advice_banking_payer").hide()
...
function payment_advice_table_row_click(el){
	...
	$.ajax(
			{
				...
				success: function (json_data) {
					...
					var banking_payer_id = data[0].banking_payer_id
					...
					$("#payment_advice_banking_payer_id").val(banking_payer_id).trigger('change');
	...
	if (paymode == 'CASHH')
	{
		...
		$("#payment_advice_banking_payer_id").val(0).trigger('change');
	...
	if (paymode == 'ONLIN'){
		...
		$("#div_payment_advice_banking_payer").show()
...
function payment_advice_create_function()
{
	...
	banking_payer_id=payment_advice_form_value['payment_advice_banking_payer_id']; 
	...
	if(payment_mode_code == 'ONLIN'){
		...
		payment_advice_form_value['banking_payer_id'] = payment_advice_form_value['payment_advice_banking_payer_id'];
		...
		if(branch_name){
			if(branch_name) {
				...
				$("#branch_name_alert").html("Branch Code should not empty")
				return false
			}
		}else{
			$("#branch_name_alert").html("Branch Code should not empty")
		...
		if(bank_name == '0'){
			$("#bank_name_alert").html("Bank Code should not empty")
			return false
		}else{
			payment_advice_form_value['bank_name_id'] = bank_name
		}
		if(banking_payer_id == '0'){
			$("#banking_payer_alert").html("Payer should not empty")
			return false
		}else{
			payment_advice_form_value['banking_payer_id'] = banking_payer_id
		}

		if(ifsc_code){
			payment_advice_form_value['ifsc_code'] = ifsc_code
		}else{
			$("#ifsc_code_alert").html("Transaction Code should not empty")
			return false
		}
