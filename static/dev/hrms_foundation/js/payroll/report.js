...
$("#organization_id_report").on("change", function(e) {
console.log($('#organization_id_report').select2("val"));
organization_id_report_list=$('#organization_id_report').select2("val")
		$.ajax({
			url : "/bank_format_org_based_emploee/",
			type : "POST",
			data : {'org_id_list':JSON.stringify(organization_id_report_list)},
			async:false,
		}).done( function(json_data) {
			data = JSON.parse(json_data);
			console.log(data)
			if (data){
				$('#employee_id_report').val(data.employee_ids).trigger('change')
			}
			else{
				$('#employee_id_report').val('').trigger('change')
			}
			$("#employee_id_report").attr('placeholder',"-- Select An Employee --");
			$("#organization_id_report").attr('placeholder',"-- Select An Organization --");
		});
});
