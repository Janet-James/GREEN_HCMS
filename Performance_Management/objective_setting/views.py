...
def stratergicObjectivesInsert(request):  #Added- Sindhuja-07Aug2018
    ...
    try:
	    ...
	    variable =request.POST.get(config.variable,False)
	    ...
	    print"VARIIIIIIIIIIII-------",variable 
            ...
            if strategic_objective_description and assigned_type  and objective_plan_type and kr_start_date and kr_end_date:
                if objective_id:
                    ...
                    organization_id=%s,organization_unit_id=%s,department_id=%s,employee_id=%s,objective_plan_type=%s,is_active=%s,modified_by_id=%s,modified_date=now(),start_date=%s,end_date=%s,planned_year=%s,planned_month=%s,planned_quarter=%s,variable=%s where id=%s returning id""",
                    (strategic_objective_description,assigned_type,bsc_perspective,organization,organization_unit,department,employee,objective_plan_type,True,current_user_id,start_date,end_date,year,month,quarter,variable,objective_id))
                    ...
                    organization_id,organization_unit_id,department_id,employee_id,objective_plan_type,is_active,created_by_id,created_date,start_date,end_date,planned_year,planned_month,planned_quarter,variable) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,now(),%s,%s,%s,%s,%s,%s) returning id""",
                    (strategic_objective_description,assigned_type,bsc_perspective,organization,organization_unit,department,employee,objective_plan_type,True,current_user_id,start_date,end_date,year,month,quarter,variable))
                    return_data=dictfetchall(cur)
                    if return_data:
