...
def user_task_details(request):
    ...
    try:
        if current_user_id:
            url_status= urllib.urlopen('http://10.0.1.5:9002/get_task_list/').getcode()
           # url_status=2
            if url_status== 200:
                user_details = {'user_id':current_user_id}
                url = 'http://10.0.1.5:9002/get_task_list/'
...
def user_task_summary_details(request):
    ...
    try:
        if current_user_id:
            url_status= urllib.urlopen('http://10.0.1.5:9002/get_worksummary_list/').getcode()
        #    url_status=2
            if url_status== 200:
                user_details = {"task_id":request.POST.get('task_id')}
                url = 'http://10.0.1.5:9002/get_worksummary_list/'
                data = urllib.urlencode(user_details)
                request = urllib2.Request(url, data)
                response = urllib2.urlopen(request)
                output = response.read()
                project_data = json.loads(output)
		print"RRRRRRRRRRR",project_data
                json_data['data'] = project_data
            ...
            json_data['error'] = e
            print"EEEEEEEEEE",e
            logger_obj.info("7MYNEXT User Dashboard form Load data exception"+ str(e) +" attempted by ")  
    return HttpResponse(json.dumps(json_data,cls=DjangoJSONEncoder), content_type=config.content_type_value)
            ...
            if requst_data:  
                ...
                if requst_data[0]['update_data']:
                    for i in requst_data[0]['update_data']:
                        ...
                        url = 'http://10.0.1.5:9002/insert_task_timer/'
...
#                 url = 'http://192.168.10.52:9001/insert_task_timer/'
		...
		print "Dattttttt",state_data
                ...
                if state_data['status']=='NTE-01':
                    ...
                    url = 'http://10.0.1.5:9002/mygreen_insert_work_summary/'
...
#         url_status= urllib.urlopen('http://192.168.11.25:8001/get/project_drop_down/').getcode()
        url_status= urllib.urlopen('http://10.0.1.5:9002/mynext_project_filter/').getcode()
#	url_status = 0
        if url_status== 200 or url_status!=200:
            app = 'application/json'
            content = 'Content-Type'
            user_details['user_id'] = current_user_id
            url = 'http://10.0.1.5:9002/mynext_project_filter/'
    ...
    try:
        if current_user_id:
            ...
            if request_data:
                app = 'application/json'
                content = 'Content-Type'
                url_status= urllib.urlopen('http://10.0.1.5:9002/project/task/mynext/list/').getcode()
#		url_status = 2
...
#                     url = 'http://192.168.11.25:8001/get/task_list/'
                    url = 'http://10.0.1.5:9002/project/task/mynext/list/'
...
def task_stage_update(request):
    ...
    try:
        if current_user_id:
            ...
            app = 'application/json'
            content = 'Content-Type'
            url_status = urllib.urlopen('http://10.0.1.5:9002/task_stage_update/').getcode()
#	    url_status = 0
            if url_status== 200 or url_status!= 200:
                url = 'http://10.0.1.5:9002/task_stage_update/'
                req = urllib2.Request(url, json.dumps(json_data), {content: app})
                f = urllib2.urlopen(req, timeout=10)
                output = f.read()
                state_data = json.loads(output)
...
#     try:
    if current_user_id:
          ...
          req_url="http://10.0.1.8:9001/scheduled_meeting/"
          req_url1="http://10.0.1.8:9001/completed_meeting/"
#	  req_url = ''
#	  req_url1 = ''
          if req_url!='':
